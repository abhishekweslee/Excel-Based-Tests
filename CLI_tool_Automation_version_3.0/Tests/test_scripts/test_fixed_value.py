...
def test_case():
    """Main test case for CLI automation."""
    # Initialize CLI automation object and logger
    cli = CLTAuto()

    # Load JSON data
    data = load_json(Paths.pattern_path)

    # Retrieve test IDs and set up logger
    test_ids = cli.get_test_ids()
    if not test_ids:
        pytest.fail("No test IDs retrieved from CLI automation.")
    log = get_logger(test_ids[0])
    log.info(f"Retrieved Test IDs: {test_ids}")

    # Retrieve test data for the first test ID
    test_data = cli.get_data(test_ids[0])
    if not test_data:
        pytest.fail(f"No test data found for Test ID: {test_ids[0]}")
    ...
    inputs = [
        ...
        (test_ids[0], data['Ticket_Details']['File_Name']),
    ...
    try:
        ...
        ticket_file_path = os.path.join(Paths.ticket_dir, f"{test_ids[0]}.txt")
